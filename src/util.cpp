...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Zblock";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Zblock.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No Zblock.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override Zblock.conf
